...
#define DEFAULT_SETTINGS                                                                           \
  (Settings) {                                                                                     \
    .tone_value = 19,     /* Default tone frequency: 12000 / 20 = 600 Hz */                        \
        .tone_volume = 3, /* Default tone volume: About half-way */                                \
        .dah_time = TIMER1_FREQUENCY * 3ul / 20ul /* Default dah time: 150ms */                    \
  }

Settings settings = DEFAULT_SETTINGS;

void reset_settings(void) { settings = DEFAULT_SETTINGS; }

#define SETTINGS_PASSWORD 87
#define PASSWORD_ADDR (void*)(0x1080)
#define SETTINGS_ADDR (void*)(0x1080 + 2)
_Static_assert((uint16_t)SETTINGS_ADDR + sizeof(settings) < 0x10BF,
